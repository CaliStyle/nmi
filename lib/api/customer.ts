...
export const Customer = {
   ...
   */
  create: function(customer, direct = false) {

    if (!direct) {
      return this.request({'add-customer': customer})
    }
    else {
      return this.direct(customer)
    }
  },
  ...
  /**
   * Execute a customer given a token
  ...
  /**
   * Retrieve all customer billing profiles
   * @param {string} customerId   customer vault id
   */
  getAllBilling: async function(customerId) {
    let res = await this.query({
      customer_vault_id: customerId,
      report_type: 'customer_vault',
      result_limit: 1,
   ...
   */
  getBilling: async function(customerId, billingId) {
    let res = await this.query({
      customer_vault_id: customerId,
      report_type: 'customer_vault',
